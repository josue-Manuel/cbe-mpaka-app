import { Settings as SettingsIcon, Moon, Globe, CircleHelp, Info, Bell, BellRing, Sun, User, Shield, Trash2, ChevronRight, CloudOff, RefreshCw, Download, Smartphone } from 'lucide-react';
...
export default function Settings() {
  ...
  const handleInstall = () => {
    if (deferredPrompt) {
      ...
      alert("Pour installer l'application, utilisez le menu de votre navigateur (trois points) et choisissez 'Ajouter à l'écran d'accueil'.");
    }
  };
  ...
  const handleClearCache = () => {
    // Keep theme and profile, clear app data to simulate cache clearing
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-6 bg-[#F8FAFC] dark:bg-[#0A192F] min-h-screen pb-24">
      ...
      <Card variant="elevated" className="border-0 p-2 dark:bg-slate-800 dark:border-slate-700 mb-6 shadow-sm">
        ...
        <div className="flex items-center justify-between p-4">
          ...
          <div className="relative inline-block w-12 h-6 align-middle select-none transition duration-200 ease-in">
            ...
            <label 
              htmlFor="dark-mode" 
              className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ${theme === 'dark' ? 'bg-[#2563EB]' : 'bg-slate-300 dark:bg-slate-600'}`}
            >
              <span 
                className={`absolute block w-4 h-4 mt-1 ml-1 rounded-full bg-white shadow-sm transform transition-transform duration-200 ${theme === 'dark' ? 'translate-x-6' : 'translate-x-0'}`}
              />
            </label>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {typeof Notification !== 'undefined' && Notification.permission === 'denied' && (
                <span className="text-[10px] text-red-500 block mt-1">Permission bloquée par le navigateur</span>
              )}
            </div>
          </div>
          <div className="relative inline-block w-12 h-6 align-middle select-none transition duration-200 ease-in">
            ...
            <label 
              htmlFor="notif-mode" 
              className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ${isPushEnabled ? 'bg-[#2563EB]' : 'bg-slate-300 dark:bg-slate-600'}`}
            >
              <span 
                className={`absolute block w-4 h-4 mt-1 ml-1 rounded-full bg-white shadow-sm transform transition-transform duration-200 ${isPushEnabled ? 'translate-x-6' : 'translate-x-0'}`}
              />
            </label>
          </div>
        </div>
        ...
        {isPushEnabled && (
          <div className="px-4 pb-4">
            <button 
              ...
              className="w-full bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 text-blue-600 dark:text-blue-400 text-sm font-medium py-3 rounded-xl flex items-center justify-center gap-2 transition-colors"
            >
              <BellRing size={16} />
        ...
        <div className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl transition-colors cursor-pointer">
          <div className="flex items-center gap-4">
            ...
            <span className="font-semibold text-slate-800 dark:text-white">Langue</span>
          </div>
          <span className="text-sm font-medium text-slate-500 dark:text-slate-400">Français</span>
        </div>
      </Card>
      ...
      <Card variant="elevated" className="border-0 p-2 dark:bg-slate-800 dark:border-slate-700 mb-6 shadow-sm">
        ...
        <div 
          ...
          className="flex items-center justify-between p-4 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-xl transition-colors cursor-pointer"
        >
          <div className="flex items-center gap-4">
      ...
      <Card variant="elevated" className="border-0 p-2 dark:bg-slate-800 dark:border-slate-700 shadow-sm mb-6">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <span className="font-semibold text-slate-800 dark:text-white">Installer l'application</span>
          </div>
          <ChevronRight size={18} className="text-slate-400" />
        </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${updateStatus === 'available' ? 'bg-orange-50 text-orange-600 dark:bg-orange-900/20 dark:text-orange-400' : 'bg-slate-50 text-slate-600 dark:bg-slate-700 dark:text-slate-400'}`}>
                <RefreshCw size={20} className={updateStatus === 'checking' ? 'animate-spin' : ''} />
              </div>
              <div>
                <span className="font-semibold text-slate-800 dark:text-white block">Mise à jour</span>
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  {updateStatus === 'idle' && 'Vérifier les mises à jour'}
                  {updateStatus === 'checking' && 'Vérification en cours...'}
                  {updateStatus === 'available' && `Nouvelle version v${latestVersion} disponible !`}
                  {updateStatus === 'up-to-date' && 'Votre application est à jour.'}
                  {updateStatus === 'error' && 'Erreur lors de la vérification.'}
                </span>
              </div>
            </div>
            ...
            {updateStatus === 'available' ? (
              <button 
                onClick={applyUpdate}
                className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-full flex items-center gap-2 transition-all shadow-md shadow-blue-600/20"
              >
                <Download size={14} />
                Installer
              </button>
              ...
              <button 
                ...
                className="text-blue-600 dark:text-blue-400 text-xs font-bold px-4 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-full transition-colors disabled:opacity-50"
              >
                Vérifier
              </button>
            )}
